...
def train(model, train_set, optimizer, seed, scheduler=None, batch_size=32, fp16=False):
...
def initialize_and_train(task_config,
    ...
    epoch = 1
    previous_dev_f1, previous_test_f1 = 0.0, 0.0
    epochs_counter = 0.0
        ...
        if dev_f1 > 1e-6:
            ...
            if hp.save_model:
                if dev_f1 > best_dev_f1:
                    best_dev_f1 = dev_f1
                    if not os.path.exists(run_tag[:-1]):
                        os.makedirs(run_tag[:-1])
